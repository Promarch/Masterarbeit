"""Prints the readings of a Bota Systems Serial sensor and manipulate the MuJoCo model with it.
to start please first enable the port:
------------------------------------------------
sudo chown robotlab /dev/ttyUSB0
------------------------------------------------
Then run: python bota_serial_example.py 
"""
import struct
import time
import threading
        ...
        try:
            with mujoco.viewer.launch_passive(m, d) as viewer:
                ...
                while viewer.is_running():
                    SCALE = 2
                    ft_in = np.array([
                        self._fx, self._fy, self._fz, self._mx, self._my, self._mz
                    ])
                    ft_in *= SCALE

                    d.xfrc_applied[3, :] = ft_in
                    mujoco.mj_step(m, d)
                    viewer.sync()
                    print('Run my loop')
